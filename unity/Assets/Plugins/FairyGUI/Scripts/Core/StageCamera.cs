...
namespace FairyGUI
{
    ...
    public class StageCamera : MonoBehaviour
    {
        ...
        private static bool isStagePositionSet;
        ...
        {
            if (isStagePositionSet)
                return;

            GameObject cameraObject = GameObject.Find(Name);
            Camera camera = cameraObject.GetComponent<Camera>();
            cameraObject.transform.position = new Vector3(GetCameraWidth(camera) / 2, -camera.orthographicSize, 0);
            isStagePositionSet = true;

        }

        public static float GetCameraWidth(Camera camera)
        {
            // The viewport coordinates for the left and right edges at the center of the view
            Vector3 leftEdgeViewport = new Vector3(0, 0.5f, camera.nearClipPlane);
            Vector3 rightEdgeViewport = new Vector3(1, 0.5f, camera.nearClipPlane);

            // Convert the viewport coordinates to world positions
            Vector3 leftEdgeWorld = camera.ViewportToWorldPoint(leftEdgeViewport);
            Vector3 rightEdgeWorld = camera.ViewportToWorldPoint(rightEdgeViewport);

            // Calculate the distance between the left and right world positions
            float width = Vector3.Distance(leftEdgeWorld, rightEdgeWorld);
