...
namespace tracer
{
    ...
    public class UpdateSenderModule : NetworkManagerModule
    {
        ...
        protected override void Init(object sender, EventArgs e)
        {
            ...
            m_controlMessages = new NetMQMessage();
        ...
        private void lockSceneObject(object sender, SceneObject sceneObject)
        {
            byte sceneId;
            short sceneObjectId;
            lock(sceneObject){                      //can still be altered from any access that is not using lock!
        ...
        public void queueUndoRedoMessage(object sender, AbstractParameter parameter)
        {
            ...
            lock (_lock)
            {
                ...
                // parameter
                Helpers.copyArray(BitConverter.GetBytes(parameter.parent.sceneID), 0, message, 3, 1);  // SceneID
                Helpers.copyArray(BitConverter.GetBytes(parameter.parent.id), 0, message, 4, 2);  // SceneObjectID
                Helpers.copyArray(BitConverter.GetBytes(parameter.id), 0, message, 6, 2);  // ParameterID
        ...
        public void queueResetMessage(SceneObject sceneObject)
        {
            ...
            lock (_lock)
            {
                ...
                // parameter
                Helpers.copyArray(BitConverter.GetBytes(sceneObject.sceneID), 0, message, 3, 1);  // SceneID
                Helpers.copyArray(BitConverter.GetBytes(sceneObject.id), 0, message, 4, 2);  // SceneObjectID
                m_controlMessages.Append(message);
            }
        ...
        protected override void run()
        {
            ...
            while (m_isRunning)
            {
                ...
                lock (_lock)
                {
                    ...
                    {
                        try { sender.SendMultipartMessage(m_controlMessages); } catch (Exception e) { UnityEngine.Debug.LogError("controlMsg:SendFrame " + e.ToString()); } // true not wait 
                    ...
                    if (m_parameterMessages.FrameCount > 3)
                    {
                        try { sender.SendMultipartMessage(m_parameterMessages); } catch (Exception e) { UnityEngine.Debug.LogError("modifiedParameter:SendFrame " + e.ToString()); } // true not wait
        ...
        //UNITY NETWORK PROBLEM DEBUG
        public const string NETWORK_OUT_DEBUG = "<color=purple>";
        public const string NETWORK_IN_DEBUG = "<color=green>";
        public const string NETWORK_MSG_DEBUG = "<color=cyan>"; 
        public const string COLOR_END = "</color>";
        public static string LogNetworkOutgoingDebugCall(System.Reflection.MethodBase functionWeCallFrom, string additionalMsg = ""){
            //return "";
            System.Text.StringBuilder stringBuilder = new System.Text.StringBuilder(NETWORK_OUT_DEBUG);
        ...
        public static string LogNetworkIncomingDebugCall(System.Reflection.MethodBase functionWeCallFrom, string additionalMsg = ""){
            //return "";
            System.Text.StringBuilder stringBuilder = new System.Text.StringBuilder(NETWORK_IN_DEBUG);
            stringBuilder.Append(" [");
            stringBuilder.Append(functionWeCallFrom.ReflectedType.Name);    //Class Name
            stringBuilder.Append(".");
            stringBuilder.Append(functionWeCallFrom.Name);                  //Function Name from where we get called (that was triggered by a call from sender)
        ...
        public static string LogNetworkParameterDebugCall(System.Reflection.MethodBase functionWeCallFrom, string additionalMsg = ""){
            //return "";
            System.Text.StringBuilder stringBuilder = new System.Text.StringBuilder(NETWORK_MSG_DEBUG);
            stringBuilder.Append(" [");
            stringBuilder.Append(functionWeCallFrom.ReflectedType.Name);    //Class Name
            stringBuilder.Append(".");
            stringBuilder.Append(functionWeCallFrom.Name);                  //Function Name from where we get called (that was triggered by a call from sender)
