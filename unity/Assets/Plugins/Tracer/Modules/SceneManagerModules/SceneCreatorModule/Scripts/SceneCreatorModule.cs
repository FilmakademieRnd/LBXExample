...
namespace tracer
{
    ...
    public class SceneCreatorModule : SceneManagerModule
    {
        ...
        public void CreateScene(object o, EventArgs e)
        {
            ...
            Helpers.Log(string.Format("Build scene from: {0} objects, {1} textures, {2} materials, {3} nodes",
                sceneData.objectList.Count, sceneData.textureList.Count, sceneData.materialList.Count,
                sceneData.nodeList.Count));
                        ...
                        switch (matPack.shaderPropertyTypes[i])
                        {
                            ...
                            case 4:
                                ...
                                if (texID > -1 && texID < SceneTextureList.Count)
                                {
                                    ...
                                    mat.SetTextureOffset(shaderPropertyId,
                                        new Vector2(matPack.textureOffsets[texIdx * 2],
                                            matPack.textureOffsets[texIdx * 2 + 1]));
                                    mat.SetTextureScale(shaderPropertyId,
                                        new Vector2(matPack.textureScales[texIdx * 2],
                                            matPack.textureScales[texIdx * 2 + 1]));
                                }
        ...
        private void createMeshes(ref SceneManager.SceneDataHandler.SceneData sceneData)
        {
            foreach (SceneManager.ObjectPackage objPack in sceneData.objectList)
            {
                ...
                {
                    Vector3 v = new Vector3(objPack.vertices[i * 3 + 0], objPack.vertices[i * 3 + 1],
                        objPack.vertices[i * 3 + 2]);
                    vertices[i] = v;
                }

                for (int i = 0; i < objPack.nSize; i++)
                {
                    Vector3 v = new Vector3(objPack.normals[i * 3 + 0], objPack.normals[i * 3 + 1],
                        objPack.normals[i * 3 + 2]);
                    normals[i] = v;
                }
        ...
        //!
        private int createSceneGraphIter(ref SceneManager.SceneDataHandler.SceneData sceneData, Transform parent,
            int idx = 0, bool isRoot = true)
        {
        ...
        private void createSkinnedMeshes(ref SceneManager.SceneDataHandler.SceneData sceneData, Transform root)
        {
            ...
            foreach (SceneManager.CharacterPackage cp in characterList)
            {
                ...
                skeleton[0].rotation = new Quaternion(cp.boneRotation[0], cp.boneRotation[1], cp.boneRotation[2],
                    cp.boneRotation[3]);
                ...
                for (int i = 1; i < cp.skeletonMapping.Length; i++)
                {
                    if (cp.skeletonMapping[i] != -1)
                    {
                        ...
                        skeleton[i].position = new Vector3(cp.bonePosition[i * 3], cp.bonePosition[i * 3 + 1],
                            cp.bonePosition[i * 3 + 2]);
                        skeleton[i].rotation = new Quaternion(cp.boneRotation[i * 4], cp.boneRotation[i * 4 + 1],
                            cp.boneRotation[i * 4 + 2], cp.boneRotation[i * 4 + 3]);
                        skeleton[i].scale = new Vector3(cp.boneScale[i * 3], cp.boneScale[i * 3 + 1],
                            cp.boneScale[i * 3 + 2]);
                    }
                }
        ...
        private GameObject CreateObject(SceneManager.SceneNode node, Transform parentTransform)
        {
            ...
            if (!parentTransform.Find(Encoding.ASCII.GetString(node.name)))
            {
                ...
                if (node.GetType() == typeof(SceneManager.SceneNodeGeo) ||
                    node.GetType() == typeof(SceneManager.SceneNodeSkinnedGeo))
                {
                    ...
                    if (nodeGeo.geoId > -1 && nodeGeo.geoId < SceneMeshList.Count)
                    {
                        ...
                        if (node.GetType() == typeof(SceneManager.SceneNodeSkinnedGeo))
                        {
                            ...
                            Bounds bounds = new Bounds(
                                new Vector3(sNodeGeo.boundCenter[0], sNodeGeo.boundCenter[1], sNodeGeo.boundCenter[2]),
                                new Vector3(sNodeGeo.boundExtents[0] * 2f, sNodeGeo.boundExtents[1] * 2f,
                                    sNodeGeo.boundExtents[2] * 2f));
                ...
                else if (node.GetType() == typeof(SceneManager.SceneNodeLight))
                {
                    ...
                    Debug.Log("Create Light: " + nodeLight.name + " of type: " + nodeLight.lightType.ToString() +
                              " Intensity: " + nodeLight.intensity + " Pos: " + pos);
        ...
        //! @param idx The index for referencing into the node list.
        //!
        private int createSkinnedRendererIter(ref SceneManager.SceneDataHandler.SceneData sceneData, Transform parent,
            int idx = 0)
        {
