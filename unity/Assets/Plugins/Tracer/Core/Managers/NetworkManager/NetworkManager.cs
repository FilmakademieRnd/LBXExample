...
namespace tracer
{
    ...
    public class NetworkManager : Manager
    {
        ...
        // [MINO]
        public event EventHandler<EventArgs> networkReady;
        //!
        //! number to count all required network modules
        //!
        private int m_networkReady = 0;

        private static readonly int requiredModules = 2;
        ...
        public NetworkManager(Type moduleType, Core vpetCore) : base(moduleType, vpetCore)
        {
            ...
            //#if UNTIY_EDITOR
            UnityEngine.Debug.Log("<color=#66FFCC>NetworkManager.NetworkManager - m_cID = " + m_cID.ToString() + "</color>");
            //#endif
        }
        ...
        // [MINO]
        //!
        //! Function that increases the network readiness level
        //!
        public void ModuleReady()
        {
            m_networkReady++;
            if (m_networkReady == requiredModules)
            {
                networkReady?.Invoke(this, EventArgs.Empty);
            }
        }
    }
}
